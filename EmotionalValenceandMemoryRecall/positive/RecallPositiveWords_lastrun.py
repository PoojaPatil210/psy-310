...
expName = 'EmotionalValenceMemoryRecall'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\POOJA\\OneDrive\\Desktop\\Stud-ious\\Sem-5\\PSY310\\PsychoPy\\EmotionalValenceandMemoryRecall\\positive\\RecallPositiveWords_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Introduction = visual.TextStim(win=win, name='Introduction',
        text='A word will appear on the screen.\nPress right arrow key if the word is abstract or left arrow key if it is concrete. ',
        font='Arial',
    ...
    # --- Initialize components for Routine "DisTask" ---
    ...
    RecallTask = visual.TextStim(win=win, name='RecallTask',
        text='Now, write down as many words as you can remember from the list you saw earlier. Take your time and try to recall as many as possible.  Press "Click to submit" after every word. \nYou have 3 minutes to complete this task.',
        font='Arial',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    trials_2 = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('poswordlist.csv'),
    ...
    for thisTrial_2 in trials_2:
        ...
        text.setText(pos_word)
        # create starting attributes for key_resp_2
        key_resp_2.keys = []
        key_resp_2.rt = []
        _key_resp_2_allKeys = []
        # keep track of which components have finished
        WordsComponents = [text, key_resp_2]
    ...
    trials_3 = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('DisTask.csv'),
    ...
    for thisTrial_3 in trials_3:
        ...
        # --- Prepare to start Routine "DisTask" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('DisTask.started', globalClock.getTime(format='float'))
        task.setText(math)
        textbox.reset()
        ...
        # keep track of which components have finished
        DisTaskComponents = [task, textbox, InstText, mouse]
        for thisComponent in DisTaskComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            # if task is stopping this frame...
            if task.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > task.tStartRefresh + 10-frameTolerance:
                    # keep track of stop time/frame for later
                    task.tStop = t  # not accounting for scr refresh
                    task.tStopRefresh = tThisFlipGlobal  # on global time
                    task.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'task.stopped')
                    # update status
            ...
            if InstText.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'InstText.started')
                # update status
            ...
            # if InstText is stopping this frame...
            if InstText.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > InstText.tStartRefresh + 10-frameTolerance:
                    # keep track of stop time/frame for later
                    InstText.tStop = t  # not accounting for scr refresh
                    InstText.tStopRefresh = tThisFlipGlobal  # on global time
                    InstText.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'InstText.stopped')
                    # update status
                    InstText.status = FINISHED
            ...
            if mouse.status == STARTED:  # only update if started and not finished!
                ...
                if buttons != prevButtonState:  # button state changed?
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        ...
                        gotValidClick = False
                        clickableList = environmenttools.getFromNames(InstText, namespace=locals())
        ...
        # --- Ending Routine "DisTask" ---
        for thisComponent in DisTaskComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('DisTask.stopped', globalClock.getTime(format='float'))
        ...
        trials_3.addData('mouse.clicked_name', mouse.clicked_name)
    ...
    _key_resp_3_allKeys = []
    # keep track of which components have finished
    RecallComponents = [RecallTask, key_resp_3]
    ...
    while continueRoutine:
        ...
        # if RecallTask is starting this frame...
        if RecallTask.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            RecallTask.frameNStart = frameN  # exact frame index
            RecallTask.tStart = t  # local t and not account for scr refresh
            RecallTask.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(RecallTask, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'RecallTask.started')
            # update status
            RecallTask.status = STARTED
            RecallTask.setAutoDraw(True)
        
        # if RecallTask is active this frame...
        if RecallTask.status == STARTED:
            # update params
            pass
    ...
    # set up handler to look after randomisation of conditions etc
    trials_4 = data.TrialHandler(nReps=25.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
    ...
    for thisTrial_4 in trials_4:
        ...
        gotValidClick = False  # until a click is received
        # keep track of which components have finished
        TestingComponents = [TestAns, Instr, mouse_2]
        ...
        # --- Run Routine "Testing" ---
        routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 7.2:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            ...
            # if TestAns is starting this frame...
            if TestAns.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                TestAns.frameNStart = frameN  # exact frame index
                TestAns.tStart = t  # local t and not account for scr refresh
                TestAns.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(TestAns, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'TestAns.started')
                # update status
                TestAns.status = STARTED
                TestAns.setAutoDraw(True)
            
            # if TestAns is active this frame...
            if TestAns.status == STARTED:
                # update params
                pass
            
            # if TestAns is stopping this frame...
            if TestAns.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > TestAns.tStartRefresh + 7.2-frameTolerance:
                    # keep track of stop time/frame for later
                    TestAns.tStop = t  # not accounting for scr refresh
                    TestAns.tStopRefresh = tThisFlipGlobal  # on global time
                    TestAns.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'TestAns.stopped')
                    # update status
                    TestAns.status = FINISHED
                    TestAns.setAutoDraw(False)
            ...
            # if Instr is stopping this frame...
            if Instr.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Instr.tStartRefresh + 7.2-frameTolerance:
                    # keep track of stop time/frame for later
                    Instr.tStop = t  # not accounting for scr refresh
                    Instr.tStopRefresh = tThisFlipGlobal  # on global time
                    Instr.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'Instr.stopped')
                    # update status
            ...
            if mouse_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > mouse_2.tStartRefresh + 7.2-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if mouse_2.status == STARTED:  # only update if started and not finished!
                ...
                if buttons != prevButtonState:  # button state changed?
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        # check if the mouse was inside our 'clickable' objects
                        gotValidClick = False
                        clickableList = environmenttools.getFromNames(Instr, namespace=locals())
                        for obj in clickableList:
                            # is this object clicked on?
        ...
        trials_4.addData('TestAns.text',TestAns.text)
        # store data for trials_4 (TrialHandler)
        trials_4.addData('mouse_2.x', mouse_2.x)
        trials_4.addData('mouse_2.y', mouse_2.y)
        trials_4.addData('mouse_2.leftButton', mouse_2.leftButton)
        trials_4.addData('mouse_2.midButton', mouse_2.midButton)
        trials_4.addData('mouse_2.rightButton', mouse_2.rightButton)
        trials_4.addData('mouse_2.time', mouse_2.time)
        trials_4.addData('mouse_2.clicked_name', mouse_2.clicked_name)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
            routineTimer.addTime(-7.200000)
        thisExp.nextEntry()
